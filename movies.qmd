...
for(i in 1:length(response_list)) {
  ...
  try({
    ...
    imdb_tomatoes$rating[i] <- data$certification
  })
}
```
...
# Need to add extraction of age rating
# …
```
...
Test extraction of age rating using purrr
```{r}
response_list[[1]]

converted_data[[8187]]$age_rating

age_ratings <- converted_data %>%
  purrr::map(~.x$certification)

age_ratings %>% length()

age_ratings %>% unlist() %>% length()

test_fun <- safely(function(x) {
  x$certification
}, otherwise = "error")

test <- converted_data %>%
  purrr::map_chr(test_fun)
```
